...
client.on('message', async msg => {
    ...
    .setTitle(':x: **You have to be in a voice channel to use this command.**'));
      ...
      .setTitle(`**Play list **${playlist.title}** Added to the queue!**`)
         ...
         .setTitle(':musical_note: Şarkını Seç!')
         .setAuthor(`${msg.author.tag}`, msg.author.avatarURL)
         .setThumbnail("https://i.postimg.cc/W1b1LW13/youtube-kids-new-logo.png")
         .setDescription(`${videos.map(video2 => `**${++index} -** ${video2.title}`).join('\n')}`)
         .setFooter('1 ila 10 arasında 1 sayı seç 10 saniyen var hızlı ol!')
         .setColor('BLACK'));
          msg.delete(5000)
    ...
    .setTitle(":x: **Birşey yazmalısın Örnek: +yardım**")
    .setColor('BLACK'));
		  return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setColor('RANDOM')
     .setTitle('Song queue')
...
client.on('message', msg => {
  ...
  if (msg.content.toLowerCase() === 'qweqweqweqwe123qweqweqwe') {
   
    msg.channel.send('**Eyw**')
