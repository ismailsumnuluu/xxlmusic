...
exports.run = function(bot, message, args) {
  ...
  message.delete(10000).catch(console.error);
    if (!message.member.voiceChannel) return message.reply("**:bangbang:Dostuuum Sana Bağlanmam İçin Bir Odaya Girmelisin :bangbang:**").then(m => m.delete(15000)).catch(console.error);
    else {
        if (!args[0] || args[0] === "help" || args[0] === "yardım") {
            message.reply("**\n \n  🎵 Radyo İstasyonları 🎵  \n Twitch:İsmaillSumnulu. \n \n 1 = Fenomen \n 2 = FenomenTürk \n 3 = KralPop \n 4 = Virgin Radio\n \n Açmak İçin +radyo <numara> \n Kapatmak İçin +radyo kapat**").then(m => m.delete(40000)).catch(console.error);
        } else if (args[0].toLowerCase() === "fenomen" || args[0] === "1") {
            message.member.voiceChannel.join().then(connection => {
                var dispatcher = connection.playStream(radyo.fenomen);
                message.reply("🎧 | **Hay Hay FenomenFM çalınıyor.**").then(m => m.delete(10000)).catch(console.error);
            })
    
            } else if (args[0].toLowerCase() === "FENOMEN TURK" || args[0] === "2") {
            message.member.voiceChannel.join().then(connection => {
                var dispatcher = connection.playStream(radyo.fenomenfm);
                message.reply("🎧 | **ELbette `FenomenTÜRK` çalınıyor.**").then(m => m.delete(10000)).catch(console.error);
            })                                                                                    
            ...
            message.member.voiceChannel.join().then(connection => {
                ...
                message.reply("🎧 | **Başarılı! :loud_sound:🎻`KralPop`🎻:loud_sound: çalınıyor.**").then(m => m.delete(10000)).catch(console.error);
            })    
            } else if (args[0].toLowerCase() === "Virgin Radio" || args[0] === "4") {
            message.member.voiceChannel.join().then(connection => {
                var dispatcher = connection.playStream(radyo.vergın);
                message.reply("🎧 | **Başarılı! :loud_sound:🎻`Virgin Radio`🎻:loud_sound: çalınıyor.** `Radyo 2 Saniye Sonra Açılıyor`").then(m => m.delete(10000)).catch(console.error);
            })   
        }
    }
    }
